...
/- [Derived consequence] The `K`-span of `Λ_D` is exactly the embedded rational
   ...
   the `K_ℝ`-span in Definition 25 (submission PDF p. 12), and is not used as a
...
/- The manuscript's successive minima use the number-field `K`-action.  This
   ...
   real printed pages used in the projection estimate. -/
...
/- The O_K-module M_n(Λ_D) from Definition 28 (submission PDF p. 14). -/
...
/- The integral matrix lattice is the finite product of the corresponding
   ...
   observation in Lemma 29 (submission PDF p. 14). -/
...
/- The inner summation set in Lemma 30 (submission PDF p. 14) is exactly the
